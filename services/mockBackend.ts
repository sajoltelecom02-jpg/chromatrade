...
export const MockAuthService = {
  login: async (phone: string, otp: string): Promise<User> => {
    ...
    if (phone === 'admin' && otp === 'admin123') {
      currentUser = { id: 'admin-1', username: 'Admin', name: 'Administrator', balance: 999999, role: 'ADMIN', phone, status: 'active' };
      return currentUser;
    }
  ...
  register: async (phone: string, password: string, name: string, username: string): Promise<User> => {
      await new Promise((resolve) => setTimeout(resolve, 800));
